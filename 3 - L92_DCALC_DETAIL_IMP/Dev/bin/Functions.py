...
def Read_insert_parameters(spark, insert_param_file,table,props, create_query_temp, query_hql, key_partition, query_itr):
    """
    ...
    for row in params:
        ...
        if param[0]==table:
            ...
            insert = hql.replace('[KP]', key_partition)
            print(insert)
            df=spark.sql(insert)
            df.createTempView(query_itr)
            # Creating a temporary partitioned table to do the overwrite to the final table (from the temporary table query)
            print("######################## QUERYYYY ########################")
            print(create_query_temp)
            print("######################## QUERYYYY ########################")
            spark.sql(create_query_temp)
            insert_statement = 'INSERT OVERWRITE TABLE '+props['hive_db_name_dest']+'.'+param[0]+'_temp'+' SELECT * FROM '+query_itr
            print("######################## INSERT ########################")
            ...
            props['partitions_for_audit']=key_partition
            
            # Get Inserted Rows Count
            #spark.conf.set("spark.sql.broadcastTimeout",  36000) # to be able to read the count 
            insert_rows_count = ' SELECT COUNT(*) AS insert_rows_count FROM '+query_itr
...
def Update_Last_2KP(spark, props, table):
    #df=spark.sql('SHOW PARTITIONS '+props['hive_db_name_dest']+'.'+'tbl_ta_notation') 
    df=spark.sql('SHOW PARTITIONS '+'l97_fermat_conso'+'.'+'tbl_v_cust_ndod_imx_stock') 
    df=df.select(substring('partition',15,26).alias('key_partitions_only_in_source'))
    kps_list = df.select('key_partitions_only_in_source').collect()
    print(kps_list)
    ...
    max_kp_but_below_odate = kps_list[-2:]
    print(max_kp_but_below_odate)
...
def Read_table_definitions(table_def_file,table,props):
    ...
    for row in params:
        param = row.split(';')
        if param[0] == table:
            table_def='CREATE EXTERNAL TABLE IF NOT EXISTS '+props['hive_db_name_dest']+'.'+param[0]+' ('+param[1]+') PARTITIONED BY (partition_key string) LOCATION '+param[2]
            table_def_temp='CREATE EXTERNAL TABLE IF NOT EXISTS '+props['hive_db_name_dest']+'.'+param[0]+'_temp'+' ('+param[1]+') PARTITIONED BY (partition_key string) LOCATION '+param[2].replace('tbl_calc_detail_imp', 'tbl_calc_detail_imp_temp')
