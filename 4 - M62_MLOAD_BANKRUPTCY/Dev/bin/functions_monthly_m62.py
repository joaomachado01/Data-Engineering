...
def select_str_for_insert(select_str,props):
    select_str=select_str.replace("[LAST_MONTH_WORKING_DAY_0000]", props["last_month_working_day_0000"]).replace("[LAST_MONTH_WORKING_DAY_0001]", props["last_month_working_day_0001"])
...
def load_final_table(spark, props, dicoTable  , audit_schema, max_col_id ):
    ...
    insert_str="INSERT OVERWRITE TABLE "+props["hivetable"]+" PARTITION (key_partition=\'"+props["monthly_partition"]+"\') SELECT * FROM "+props["hive_db"]+"."+"wrk_"+props["HiveTableForWorking"]
    spark.sql(insert_str)
    print("FINAL TABLE WAS LOADED WITH PARTITION: "+props["monthly_partition"])
    ##########COUNTS NUMBER OF INSERTED ROWS
    countQuery ="( select  count(*) as cnt from {0} WHERE key_partition=\'{1}\')".format( props['HiveTabel'],props["monthly_partition"] )
