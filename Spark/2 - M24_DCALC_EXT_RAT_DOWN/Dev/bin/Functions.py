...
def Read_insert_parameters(spark, insert_param_file, table, props, create_query_temp, query_hql):
    """
    ...
    for row in params:
        param = row.split(';')
        if param[0]==table:
              last_kp = Get_Max_KP_But_Below_Odate(spark, props, table)
              ...
              insert = hql.replace('CURRENT_KP', props['partition_to_out']).replace('PREVIOUS_KP', last_kp)
              print(insert)
              df=spark.sql(insert)
...
def Get_KPs_Below_Or_Equal_Odate(partitions_list, props):
...
def Read_table_definitions(table_def_file,table,props):
    ...
    for row in params:
        ...
        if param[0] == table:
            ...
            table_def_temp='CREATE EXTERNAL TABLE IF NOT EXISTS '+props['hive_db_name_dest']+'.'+param[0]+'_temp'+' ('+param[1]+') PARTITIONED BY (key_partition string) LOCATION '+param[2].replace('tbl_sqr_deg_not_ext', 'tbl_sqr_deg_not_ext_temp')
