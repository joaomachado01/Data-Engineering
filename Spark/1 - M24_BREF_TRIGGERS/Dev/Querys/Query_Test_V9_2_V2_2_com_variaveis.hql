...
(
                    ...
                    where flg_etat = 'V' and cod_frequence >= 12 and cod_vision in ('C','S') and key_partition = 'KP' 
                    and fs.dat_periode = (
                                            ...
                                            and fm.cod_vision = fs.cod_vision and key_partition = 'KP' and flg_etat = 'V' 
...
(
    ...
    AND UPPER(cod_compte) in (turnovers)
), 
...
(
    ...
    CASE 
        WHEN (ammount_.has_ammount = 1 AND dat_arrete > key_partition_modified_minus_18 AND ammount_.fiches_count <> check_.fiches_count_check) OR (ammount_.has_ammount = 1 AND dat_arrete > key_partition_modified_minus_18 AND ammount_.fiches_count = check_.fiches_count_check) THEN 'RULE2&3'
    ...
    END rule1,
    CASE 
        WHEN (ammount_.has_ammount = 1) AND (dat_arrete > key_partition_modified_minus_18) AND (ammount_.fiches_count <> check_.fiches_count_check) THEN RANK() OVER(PARTITION BY detail_.cod_raf ORDER BY detail_.mnt_compte_v2 DESC) --Rule 2
        ELSE NULL
    END rule2,
    CASE 
        WHEN (ammount_.has_ammount = 1) AND (dat_arrete > key_partition_modified_minus_18) AND (ammount_.fiches_count = check_.fiches_count_check) THEN RANK() OVER(PARTITION BY detail_.cod_raf ORDER BY detail_.dat_arrete_v2 DESC, detail_.dat_maj_v2 DESC, detail_.cod_vision_v2 DESC)  -- Rule 3
        ELSE NULL
    END rule3
